...
describe('Increment 31: Observation Builder', () => {
  ...
  describe('return type', () => {
    it('returns object with obs Float32Array and selectedAsteroids Set', () => {
      const result = buildObservation(ship(), ship({ x: 100 }), []);
      expect(result.obs).toBeInstanceOf(Float32Array);
      expect(result.obs.length).toBe(OBSERVATION_SIZE);
      expect(result.selectedAsteroids).toBeInstanceOf(Set);
    });
  });
  ...
  describe('asteroid observations', () => {
    ...
    it('asteroids sorted by distance (nearest first)', () => {
      const { obs } = buildObservation(
        ship({ heading: 0 }),
        ship({ x: 1000 }),
        [asteroid(300, 0), asteroid(100, 0), asteroid(200, 0)],
      );
  ...
  // -----------------------------------------------------------------------
  // selectedAsteroids (Increment 44)
  // -----------------------------------------------------------------------

  describe('selectedAsteroids', () => {
    it('is empty Set when no asteroids exist', () => {
      const { selectedAsteroids } = buildObservation(
        ship(),
        ship({ x: 100 }),
        [],
      );
      expect(selectedAsteroids).toBeInstanceOf(Set);
      expect(selectedAsteroids.size).toBe(0);
    });

    it('is empty Set when all asteroids are out of range', () => {
      const { selectedAsteroids } = buildObservation(ship(), ship({ x: 100 }), [
        asteroid(1500, 0),
        asteroid(2000, 500),
      ]);
      expect(selectedAsteroids.size).toBe(0);
    ...
    it('contains the exact asteroid objects within range', () => {
      const a1 = asteroid(200, 0);
      const a2 = asteroid(400, 0);
      const { selectedAsteroids } = buildObservation(ship(), ship({ x: 100 }), [
        a1,
        a2,
      ]);
      expect(selectedAsteroids.size).toBe(2);
      expect(selectedAsteroids.has(a1)).toBe(true);
      expect(selectedAsteroids.has(a2)).toBe(true);
    });

    it('contains only nearest k when more than k asteroids exist', () => {
      const rocks = [];
      for (let i = 1; i <= 12; i++) {
        rocks.push(asteroid(i * 50, 0)); // 50, 100, ..., 600
      }
      const { selectedAsteroids } = buildObservation(
        ship(),
        ship({ x: 1000 }),
      ...
      // Only nearest 8 within 1000px range
      expect(selectedAsteroids.size).toBe(8);
      // Nearest 8 are at distances 50..400 (indices 0..7)
      for (let i = 0; i < 8; i++) {
        expect(selectedAsteroids.has(rocks[i])).toBe(true);
      }
      // Farther ones excluded
      for (let i = 8; i < 12; i++) {
        expect(selectedAsteroids.has(rocks[i])).toBe(false);
      }
    });

    it('has correct count when fewer than k asteroids in range', () => {
      const a1 = asteroid(100, 0);
      const a2 = asteroid(300, 0);
      const a3 = asteroid(1500, 0); // out of range
      const { selectedAsteroids } = buildObservation(ship(), ship({ x: 100 }), [
        a1,
        a2,
        a3,
      ...
      expect(selectedAsteroids.has(a1)).toBe(true);
      expect(selectedAsteroids.has(a2)).toBe(true);
      expect(selectedAsteroids.has(a3)).toBe(false);
    });

    it('same asteroids used for obs vector and selectedAsteroids', () => {
      const near = asteroid(200, 0);
      const far = asteroid(1500, 0);
      const { obs, selectedAsteroids } = buildObservation(
        ship({ heading: 0 }),
        ship({ x: 500 }),
        [near, far],
      );
      // obs should have one asteroid at distance 200/1000=0.2
      expect(obs[12]).toBeCloseTo(0.2, 4);
      // Second slot should be zero (far asteroid excluded)
      expect(obs[15]).toBe(0);
      // selectedAsteroids should match
      expect(selectedAsteroids.has(near)).toBe(true);
      expect(selectedAsteroids.has(far)).toBe(false);
    });
  });
});
