...
**Fix**: Add `_emergencyBreakStreak` counter to AI state; when streak >= EMERGENCY_BREAK_LIMIT, skip the emergency break and run `selectBestAction` as if the hold timer expired. Streak increments on each emergency break, decays by 1 on non-emergency re-evaluations.
**Complexity**: 3 — New behavioral mechanism (circuit breaker counter)

### Sweep (50 games each, KILL-event-based win counter)

| LIMIT | Wins/50 | Win% | Osc/game | Collapse/game | Fires/game |
|-------|---------|------|----------|---------------|------------|
| 1 | 24 | 48% | 2.84 | 2.04 | 4.7 |
| 2 | 24 | 48% | 3.60 | 2.12 | 3.6 |
| 3 | 24 | 48% | 2.38 | 1.16 | 3.4 |
| 4 | 25 | 50% | 2.66 | 1.90 | 4.1 |
| **5** | **29** | **58%** | **2.78** | **1.88** | **2.9** |
| 6 | 23 | 46% | 2.72 | 1.50 | 3.4 |
| 999 (unlimited) | 20 | 40% | 2.02 | 1.28 | 3.2 |

Selected: LIMIT=5 (highest wins in sweep).

### 200-Game Validation (LIMIT=5)
Player wins: 80/200 | Enemy wins: 120/200 | Draws: 0/200
Oscillations: 2.68/game | Collapses: 1.755/game | Fires/game: 3.2

### Decision
ROLLBACK — 80/200 wins << 110 threshold.

Key observations:
- LIMIT=5 won the 50-game sweep with 29/50 (58%) but completely inverted at 200 games to 80/200 (40%)
- The -30-win swing from 50-game to 200-game is the largest variance artifact seen in any cycle
- Circuit breaker suppresses genuine emergency breaks: when the AI is in a rapid-flip state, the underlying threat is often still real; scoring selection doesn't find a better exit path
- Lower LIMIT values (1, 2, 3) all clustered at exactly 24/50 wins (48%) — no improvement in that range
- Unlimited (LIMIT=999) produced only 20/50 — the circuit breaker mechanism itself appears to add noise to the action selection
- The fundamental problem: when `hasImminentCollision` fires repeatedly, it's because the trajectory genuinely keeps hitting the asteroid, not just a false positive. Suppressing the break doesn't fix the geometry.

**Key insight**: The circuit breaker approach assumes the emergency break is a false positive being triggered repeatedly. But the data suggests repeated breaks fire because the AI is trapped in a geometry where no available action clears the asteroid within 3 steps. Suppressing the break and running scored selection produces the same scored trajectory that caused the original break — scored selection also detects the asteroid and produces a heavily-penalized result. The net effect is behavioral noise without geometric improvement. The oscillation is a symptom of a navigation deadlock, not excessive sensitivity.
